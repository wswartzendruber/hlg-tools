...
fn main() {
    ...
    let matches = app_from_crate!()
        ...
        .after_help(format!("This utility follows the BT.2390 method for generating a \
            PQ-to-HLG conversion LUT. The linear input brightness will first be scaled to \
            bring the --ref-white level to 203 nits, if necessary. This will cause the \
            --max-cll value to be internally adjusted as well. If the internal MaxCLL value \
            then exceeds 1,000 nits, BT.2390 luminosity tone mapping will be applied to \
            compress the input to 1,000 nits. From there, the signal will be converted to \
            HLG. The generated LUTs are completely full range.\n\n\
            Copyright © 2021 William Swartzendruber\n\
            Licensed under the Open Software License version 3.0\n\
    ...
    let pq_hlg_mapper = PqHlgMapper::new(ref_white, max_cll);
